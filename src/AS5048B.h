...
#define USE_WIREBEGIN_ENABLED // to comment if Wire.begin() function is called in Setup() for instance. Usefull to
                              // manage one or several I2C devices in the same sketch
...
#define EXP_MOVAVG_N                                                                                                   \
    5 // history length impact on moving average impact - keep in mind the moving average will be impacted by the
      // measurement frequency too
#define EXP_MOVAVG_LOOP                                                                                                \
    1 // number of measurements before starting mobile Average - starting with a simple average - 1 allows a quick
      // start. Value must be 1 minimum
...
class AS5048B
{
  ...
  public:
    ...
    // methods
    uint8_t readReg8(uint8_t);
    uint16_t readReg16(uint8_t); // 16 bit value got from 2x8bits registers (7..0 MSB + 5..0 LSB) => 14 bits value
    void writeReg(uint8_t, uint8_t);
    double convertAngle(int, double); // RAW, TRN, DEG, RAD, GRAD, MOA, SOA, MILNATO, MILSE, MILRU
    ...
    double angleR(int unit = U_RAW, bool newVal = true); // Read current angle or get last measure with unit conversion
